...
cluster = mongodb(name="mongo", username="root", password="secret", wired_tiger_cache_gb=1)

collection_nodes = []
for database in DATABASES:
    for collection in COLLECTIONS.get(database, []):
        n = create_collection(cluster=cluster, db=database, collection=collection)
...
for spec_key, index_list in INDEXES.items():
    ...
    for keys in index_list:
        n = create_index(cluster=cluster, db=database, collection=collection, keys=keys, after=collection_nodes)
...
migrate = run_script(cluster=cluster, db="analytics", script_path="migrations/001_partition_events.js", after=[seed])

app = service.run(
    name  = "catalog-api",
    env   = {"MONGO_URI": "mongodb://root:secret@" + cluster.name + ":27017"},
    after = [migrate],
)
