...
CHANNEL_CONFIGS = {
    "email":   {"partitions": 6,  "mock_def": "mock/email",   "image": "python:3.12"},
    "sms":     {"partitions": 4,  "mock_def": "mock/sms",     "image": "python:3.12"},
    "push":    {"partitions": 8,  "mock_def": "mock/push",    "image": "python:3.12"},
...
seed_templates = task.run(
    name     = "seed-templates",
    image    = "postgres:16",
    commands = ["psql \"${DATABASE_URL:-postgresql://postgres:postgres@db:5432/app}\" -c \"SELECT 1\" -q"],
    after    = [migrations],
)
...
for channel in CHANNELS:
    ...
    events_topic = create_topic(broker, "notifications-" + channel,   partitions=cfg["partitions"])
    channel_topics[channel] = [events_topic]

    if ENABLE_DLQ:
        dlq_topic = create_topic(broker, "notifications-" + channel + "-dlq", partitions=1)
